...
void DrawTypeName (Allegro *allegro)
{
    ...
    {
        al_draw_textf(allegro->font_24, al_map_rgb(255, 255, 255), 50 + j*20, 500, 1, "%c", allegro->name[j]);
    }
...
void DrawScoreBoard (Allegro *allegro)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        al_draw_textf(allegro->font_24, al_map_rgb(255, 255, 255), 300, 200 + i * 50, 0, "%s", allegro->player[i].name);
    }
}
...
void CheckKeyboardDown(Allegro *allegro)
{
    if (allegro->STATE == TYPE)
    {
        ...
        case ALLEGRO_KEY_BACKSPACE:
            ...
            break;
        case ALLEGRO_KEY_A:
            if(allegro->name_length >= 20) break;
